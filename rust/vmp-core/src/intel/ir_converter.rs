...
impl IrConverter {
    ...
    fn convert_call(&self, instruction: &Instruction) -> DisassemblyResult<Option<IrOpcode>> {
        let target = if let Some(addr) = instruction.branch_target() {
            eprintln!("[DEBUG ir_conv] call Direct branch_target=0x{:X}, ip=0x{:X}", addr, instruction.ip());
            IrJumpTarget::Direct(addr)
        } else {
            let op = self.convert_operand(instruction, 0)?;
            match op {
    ...
    fn convert_memory_operand(&self, instruction: &Instruction, _op_index: u32) -> DisassemblyResult<IrMemoryOperand> {
        ...
        if iced.memory_base() == IcedRegister::RIP {
            eprintln!("[DEBUG ir_conv] RIP-relative mem: ip=0x{:X}, next_ip=0x{:X}, disp32=0x{:X}, memory_displacement64=0x{:X}",
                iced.ip(), iced.next_ip(), iced.memory_displacement32(), disp);
        }
        Ok(IrMemoryOperand {
